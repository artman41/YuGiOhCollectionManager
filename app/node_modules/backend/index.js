...
global.storage = []; //all the cards the player owns
global.cardCollection = null; //the collection of cards in the center pane
global.ft = null; //the filter, right-most textbox
global.currentTab = 0;

global.IndexJson = null; //the global card index file
var extras = ["Xyz", "Fusion", "Synchro", "Pendulum"]; //types that are meant to be in the extra deck

var dirtyStorage = false;

function SaveStorage(force) {
    if (force !== undefined && force) {
        fs.writeFile(`${__approot}/.storage`, storageString, function (err) {
            if (err == undefined)
    ...
    setTimeout(() => {
        ...
        for (var i = 0; i < storage.length; i++) {
            ...
            for (var j = 0; j < item.amount; j++) {
                storageString += `${item.number}${item.comment != "" ? " #" + item.comment : ""}\n`;
            }
        }
...
function LoadStorage() {
    fs.readFile(`${__approot}/.storage`, 'utf-8', function (err, data) {
        ...
        for (var i = 0; i < numbers.length; i++) {
            var splitNumber = numbers[i].split("#");
            var sanitisedNumber = splitNumber[0];
            var comment = splitNumber[1];

            //gets the indexed card json from IndexJson
            var jsonI = IndexJson.find(o => parseInt(o.cardNumber) == parseInt(sanitisedNumber));

            //gets the card from the collection
            var tempCard = GetCard(jsonI.cardType, parseInt(sanitisedNumber));

            //adds the card to the deck
            var paramObj = new objects.ParamObject();
            paramObj.number = tempCard.number;
            paramObj.args = [1, comment];

            LoadJsonFile(AddCardToStorage, paramObj);
        }
...
function SaveDeck() {
    ...
    var extraDeck = SortDeck(deck.filter(o => JSON.parse(o.json["subCategories"]).contains(extras))); //gets the extra Deck cards
    ...
    for (var i = 0; i < mainDeck.length; i++) {
        var item = mainDeck[i];
        for (var j = 0; j < item.amount; j++) {
            deckString += `${item.json.cardNumber}${item.comment != "" ? " #" + item.comment : ""}\n`;
        }
    }
    //----------------------------------------
...
async function InitializeCardCollection(indexJson) {
...
function GetCard(cardType, number) {
    return collection.categories[cardType - 1].cards.find(o => parseInt(o.number) == parseInt(number));
}

//adds a card to the deck
function AddCard(json, amount, comment) {
    //attempts to find the card within the deck
    var item = deck.find(o => o.number == json["cardNumber"]);
    //if the card can't be found
    if (item == null) {
        //----------------------------------------
        var x = new objects.Card(json, amount, comment);
        deck.push(x);
        //console.log(x.number, x.name);
        eventcollection.Card_Added(deck[deck.length - 1]);
        //----------------------------------------
...
function AddCardToStorage(json, comment) {
    //attempts to find the card within the deck
    var item = storage.find(o => o.number == json["cardNumber"]);
    //if the card can't be found
    if (item == null) {
        //----------------------------------------
        var x = new objects.Card(json, 1, comment);
        storage.push(x);
        //console.log(x.number, x.name);
        eventcollection.Card_Added(storage[storage.length - 1]);
        //----------------------------------------
        //| Creates a card object and pushes it
        //|  to the deck array
        //----------------------------------------
    ...
    var x = GetCard(json.cardType, json.cardNumber);
    //console.log(x);
    frontend.UpdateShownCard(x);
    frontend.UpdateShownCollection(null);
    SaveStorage();
}

function RemoveCardFromStorage(json) {
//attempts to find the card in the deck
    var item = storage.find(o => o.number == json["cardNumber"]);
    //if the card is in the deck
    if (item != null) {
        ...
        //if the amount of cards <= 0 then remove it
        if (item.amount <= 0) {
            storage = storage.filter(o => o !== item);
            frontend.UpdateShownCollection(null);
            frontend.UpdateShownDeck();
        }
        eventcollection.Card_Removed(item);
    }
    frontend.UpdateShownCard(GetCard(json.cardType, json.cardNumber));
}
