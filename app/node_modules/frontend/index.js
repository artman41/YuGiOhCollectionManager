...
function UpdateShownCollection(ev) {
    ...
    var loop = function (childNode) {
        if (filterText == "" || ( //if filter text is empty
            ...
            else //else if the span is hidden
            if (currentTab == 1) {
                if (storage.map(o => o.number).includes(childNode.number)) {
                    childNode.style.display = "block"; //unhide the span
                }
            }
            else
                childNode.style.display = "block"; //unhide the span
            if (storage.map(o => o.number).includes(childNode.number) && currentTab == 0)
                childNode.style.backgroundColor = "#8de490";
            else
...
function UpdateShownDeck() {
    ...
    for (var i = 0; i < deck.length; i++) {
        ...
        navSpan.type = "span";
        navSpan.secret = "deckSpan";
        if (!storage.map(o => o.number).includes(deck[i].number))
            navSpan.style.backgroundColor = "rgb(247, 127, 130)";//"#b14949";
        else
            navSpan.style.backgroundColor = "";
        navSpan.addEventListener("contextmenu", e => eventcollection.ContextMenuHandler(e.currentTarget, e));
        //----------------------------------------
        ...
        navSpan.onclick = e => {
            UpdateShownCard(backend.GetCard(e.currentTarget.object.type, e.currentTarget.object.number));
        };
    ...
    document.getElementById("DeckHeader").innerText = `Deck - ${deckSize} cards`
}
...
function UpdateShownCard(span) { //where span is an object created in <InitializeCardCollection>
    ...
    var callback = function (json, span) {
        ...
        var cardAmount = document.getElementById("CardStored");
        var x = storage.find(o => o.number == json["cardNumber"]);

        if (number.innerHTML != json["cardNumber"]) {
        ...
        cardAmount.innerHTML = x !== undefined ? x.amount : "0"
    };
